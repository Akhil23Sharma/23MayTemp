...
const HeaderField = (props) => {
    ...
    const getBackgroundColor = (nodeData) => {
        if (nodeData.mode !== MODES.EDIT) return 'background.nodeHeader'; // Default for non-edit mode
    
        switch (nodeData.nodeStatus) {
            case 'new':
                return '#D0F0C0'; // A light green
            case 'copied':
                return '#FFFFE0'; // A light yellow
            case 'removed':
                return '#F08080'; // A light coral
            default:
                return 'background.nodeHeader'; // Default edit mode color from theme
        }
    };
    ...
    if (props.data.mode === MODES.EDIT) {
        if (props.data.type === DATA_TYPES.ARRAY && !props.data['data-remove'] && !props.data.uiUpdateOnly) {
            add = true;
            // remove = true; // Remove button for array container itself is usually not desired for item removal
        }
    ...
    return (
        <Box className={classes.container} data-xpath={props.data.xpath}>
            <Box 
                ...
                bgcolor={getBackgroundColor(props.data)}
            >
...
const HeaderOptions = ({ add, remove, copy, show, metadata, onClick, onToggle }) => {
    const { xpath, ref } = metadata;

    // Helper to check if xpath points to an array item (child)
    const isChild = xpath.endsWith(']');
    const isArrayContainer = metadata.type === DATA_TYPES.ARRAY;

    // Determine button visibility based on your rules
    // For Array container: only show Add
    // For Object item in Array: show Copy and Remove
    // For Standalone Object: show Add (to populate if null) or Remove (to nullify if present)

    let showAddButton = false;
    let showCopyButton = false;
    let showRemoveButton = false;

    if (metadata.mode === MODES.EDIT) {
        if (isArrayContainer) {
            showAddButton = metadata['object-add']; // From treeHelper: true for arrays
        } else if (metadata.type === DATA_TYPES.OBJECT) {
            if (isChild) { // Object item in an array
                showCopyButton = metadata['object-addCopy']; // From treeHelper: true for array items
                showRemoveButton = metadata['object-remove']; // From treeHelper: true for array items
            } else { // Standalone object
                showAddButton = metadata['object-add'];
                showRemoveButton = metadata['object-remove'];
                // showCopyButton for standalone objects can be true if explicitly set by treeHelper
                showCopyButton = metadata['object-addCopy']; 
            }
        }
    }


    if (showAddButton || showCopyButton || showRemoveButton) {
        if (show) {
            return (
                <ClickAwayListener onClickAway={() => onToggle(false)}>
                    <Box className={classes.menu} bgcolor='background.secondary'>
