...
function addNode(tree, schema, currentSchema, propname, callerProps, dataxpath, type, xpath) {
    ...
    // Handle object type with items (references)
    ...
    // Handle regular object type (process its properties)
    else if (currentSchemaType === DATA_TYPES.OBJECT && currentSchema.properties) {
        ...
        if (get(data, dataxpath) === undefined && get(originalData, xpath) === undefined && currentSchema.type !== DATA_TYPES.OBJECT) return;
        ...
        const childNode = addHeaderNode(
            ...
            dataxpath, 
            xpath, 
            currentSchema.items?.$ref, // This might be null for regular objects, which is fine
            headerState
        );

        // If data is null and originalData is also null or undefined, don't process properties.
        if (get(data, dataxpath) === null && (!get(originalData, xpath) || get(originalData, xpath) === null)) return;

        processMetadataProperties(currentSchema, childNode, schema, callerProps, dataxpath, xpath);
    }
    // Implicitly, if none of the above, it might be a primitive handled by addSimpleNode elsewhere, or an unhandled case.
}
...
function addHeaderNode(node, currentSchema, propname, type, callerProps, dataxpath, xpath, ref, objectState) {
    const headerNode = {
        id: xpath,
        key: xpath,
        title: currentSchema.title || propname, // Fallback to propname if title is missing
        name: propname,
        type,
        ref,
    ...
    const currentVal = get(callerProps.data, dataxpath);
    const os = objectState || determineHeaderState(callerProps.data, callerProps.originalData, xpath, currentSchema);

    // Check for transient status from DataTree.jsx (e.g., 'new', 'copied')
    if (currentVal && typeof currentVal === 'object' && currentVal.nodeStatus) {
        headerNode.nodeStatus = currentVal.nodeStatus;
        // Potentially remove it from currentVal after transferring if it's a one-time flag for tree generation
        // delete currentVal.nodeStatus; // Or handle this cleanup in DataTree after onUpdate if needed
    }

    // If the object data is null, and a remove action is/was possible, mark it as 'removed' for styling
    // This might override 'new' or 'copied' if an action sequence leads to that, which is fine.
    if (type === DATA_TYPES.OBJECT && currentVal === null && os.remove) {
        headerNode.nodeStatus = 'removed';
    }
    ...
    // Button logic based on type and context
    if (type === DATA_TYPES.ARRAY) {
        // This is an array container node
        headerNode['object-add'] = true; // Can add new items to the array
    } else if (type === DATA_TYPES.OBJECT) {
        if (xpath && xpath.includes('[') && xpath.endsWith(']')) {
            // This is an OBJECT ITEM within an array (e.g., a specific broker)
            headerNode['object-remove'] = true;  // Action: remove this item from array
            headerNode['object-addCopy'] = true; // Action: duplicate this item in array
        } else {
            // This is a STANDALONE object (e.g. root object, or a direct object property not in an array)
            // Its actions depend on whether its data is present or null (set fields to null or repopulate)
            const stateForStandaloneObject = objectState || determineHeaderState(callerProps.data, callerProps.originalData, xpath, currentSchema);
            if (stateForStandaloneObject.add) headerNode['object-add'] = true; // Action: (re)populate fields / set object from null
            if (stateForStandaloneObject.remove) headerNode['object-remove'] = true; // Action: set fields to null / nullify object
            // addCopy for standalone objects is less common, depends on schema/desire
            if (stateForStandaloneObject.addCopy) headerNode['object-addCopy'] = true; 
        }
    }
    // The original root object data-remove (to clear all data for the root model)
    // This needs to be specific to the root node, identified by not having a dataxpath initially maybe?
    // generateTreeStructure calls addHeaderNode for root with dataxpath = currentSchemaName and xpath = currentSchemaName
    // Let's assume the root object is the one where its dataxpath is its own name and it has no '['.
    if (type === DATA_TYPES.OBJECT && dataxpath === propname && xpath === propname && !(xpath && xpath.includes('[') && xpath.endsWith(']'))) {
         if (!currentSchema.orm_no_update) { // Only if not read-only
            headerNode['data-remove'] = true; // Button to clear/reset the whole model instance
         }
    }
...
function createSimpleNode(attributes, propname, dataxpath, xpath, data, currentSchema, callerProps) {
    ...
    return {
        ...
        value: get(callerProps.data, dataxpath)
    };
}
...
function processMetadataProperties(metadata, childNode, schema, callerProps, dataxpath, xpath) {
    Object.keys(metadata.properties).forEach((prop) => {
        ...
        if (metadata.required && !metadata.required.includes(prop)) {
        ...
        const childxpathProp = dataxpath ? `${dataxpath}.${prop}` : prop;
        const updatedxpathProp = `${xpath}.${prop}`;

        if (metadataProp.type === DATA_TYPES.OBJECT) {
            addNode(childNode, schema, metadataProp, prop, callerProps, childxpathProp, null, updatedxpathProp);
        } else if (primitiveDataTypes.includes(metadataProp.type)) {
            addSimpleNode(childNode, schema, metadata, prop, callerProps, childxpathProp, updatedxpathProp);
        } else {
            addNode(childNode, schema, metadataProp, prop, callerProps, childxpathProp, null, updatedxpathProp);
        }
    });
...
function isEmptyArrayData(data, originalData, dataxpath, xpath) {
    // Basic implementation, might need refinement based on actual usage
    const currentDataEmpty = !get(data, dataxpath) || get(data, dataxpath).length === 0;
    const originalDataEmpty = !get(originalData, xpath) || get(originalData, xpath).length === 0;
    // Consider it empty if both current and original are empty, or if current is empty and original didn't exist.
    return (currentDataEmpty && originalDataEmpty) || (currentDataEmpty && get(originalData, xpath) === undefined);
}
...
function processArrayItems(tree, schema, currentSchema, propname, callerProps, dataxpath, xpath) {
    ...
    if (get(originalData, xpath)) {
        for (let i = 0; i < get(originalData, xpath).length; i++) {
            ...
            childxpath = getDataxpath(data, updatedxpath); // Ensure correct childxpath if data was reordered
    ...
    if (get(data, dataxpath)) {
        get(data, dataxpath).forEach((childobject, i) => {
            // Ensure childobject is an object and has an xpath_ property before proceeding
            if (typeof childobject !== 'object' || childobject === null) return; 
            const subpropname = Object.keys(childobject).find(key => key.startsWith('xpath_'));
            if (!subpropname || !childobject[subpropname]) return; 
            
            const propxpath = childobject[subpropname];
            const propindexPart = propxpath.substring(propxpath.lastIndexOf('[') + 1, propxpath.lastIndexOf(']'));
            // Ensure propindexPart is a valid number string
            if (!propindexPart || isNaN(parseInt(propindexPart))) return;
            const propindex = parseInt(propindexPart);
            ...
            itemsToProcess.push({
                index: i, // Use the current loop index for data array
                childxpath, // Path in data array
                updatedxpath, // Path in original/logical tree structure
                fromOriginal: false
            });
            paths.push(updatedxpath); // Use updatedxpath to track processed logical paths
        });
    }
    ...
    const itemSchemaRef = currentSchema.items?.$ref;
    let itemSchemaDefinition = currentSchema; 
    let itemTitle = propname; 

    if (itemSchemaRef) {
        const refParts = itemSchemaRef.split('/');
        const resolvedSchema = refParts.length === 2 ? schema[refParts[1]] : schema[refParts[1]]?.[refParts[2]];
        if (resolvedSchema) {
           itemSchemaDefinition = resolvedSchema;
           itemTitle = getNodeTitleFromRef(itemSchemaRef, schema); 
        } else {
            itemSchemaDefinition = currentSchema; // Fallback
        }
    }
    
    if (needsPagination) {
        let parentNode = null;
        // The parent node for array items should be the one whose children array is being passed as 'tree'
        // This means 'tree' itself IS the children array of the container node.
        // We need to find the container node in the actual main tree structure if we want to add pagination controls to it.
        // This part of logic might need to be in handleArrayWithItems, where the container node is created.
        // For now, let's assume 'tree' is where items go, and pagination controls are handled by the caller or parent context.

        // Simplified: process items for current page, assuming pagination state is managed by callerProps or a component state
        const currentPage = callerProps.pagination?.[xpath]?.currentPage || 0;
        const startIndex = currentPage * ITEMS_PER_PAGE;
        const endIndex = Math.min(startIndex + ITEMS_PER_PAGE, totalItems);

        itemsToProcess.slice(startIndex, endIndex).forEach(item => {
            addNode(tree, schema, itemSchemaDefinition, itemTitle, callerProps, item.childxpath, DATA_TYPES.OBJECT, item.updatedxpath);
        });
    } else {
...
function getNodeTitleFromRef(ref, projectSchema) {
    const parts = ref.split('/');
    const refKey = parts[parts.length - 1];
    const refSchema = parts.length === 2
        ? projectSchema[refKey]
        : projectSchema.definitions?.[refKey]; // common place for definitions
    return refSchema?.title || refKey; 
}
...
function buildArrayAdditionalProps(schema, currentSchema) {
    ...
    if (currentSchema.underlying_type === DATA_TYPES.ENUM && currentSchema.items?.$ref) {
        const ref = currentSchema.items.$ref;
        const refSplit = ref.split('/');
        const metadata = refSplit.length === 2 ? schema[refSplit[1]] : schema.definitions?.[refSplit[refSplit.length-1]];
        if (metadata && metadata.enum) {
